...
def inverse_spd(M):
    r"""
    Compute the inverse of a symmetric positive-definite (SPD) matrix.
    ...
    ``QQbar`` (which contains *all* roots)::

        sage: from sage.geometry.cone_critical_angles import inverse_spd
        sage: M = matrix(QQ, [[3,1],[1,5]])
        sage: M_inv = inverse_spd(M)
        sage: M_inv.base_ring().is_subring(QQbar)
        True
        sage: I = matrix.identity(QQbar,2)
        sage: M_inv*M == I
        True
        sage: M*M_inv == I
        True
    ...
    """
    # M = L * L.transpose(). The default "echelonize" inverse() method
    # works just fine for triangular matrices.
...
def solve_gevp_nonzero(GG, HH, M, I, J):
    ...
    G_I_pinv_H_J = inverse_spd(GG[I,I]) * M[I,J]
    H_J_pinv_G_I = inverse_spd(HH[J,J]) * M.transpose()[J,I]
